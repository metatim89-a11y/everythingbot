...
OUT_DIR="$PROJECT_ROOT/LOGS/mcp_server/out"
ERR_DIR="$PROJECT_ROOT/LOGS/mcp_server/err"
...
if command -v gnome-terminal >/dev/null 2>&1 && [ -n "$DISPLAY" ]; then
    gnome-terminal --tab --title="everythingbot $SERVICE_NAME" -- bash -c "
        (cd $PROJECT_ROOT/server && node index.js) 2> >(tee -a $ERR_DIR/mcp_server_stderr.log) | tee -a $OUT_DIR/mcp_server_stdout.log
    "
else
