  ...
  return (
    <div className={`flex w-full ${isUser ? 'justify-end' : 'justify-start'} mb-4 message-bubble-container`}>
      <div className={`flex flex-col ${isUser ? 'items-end' : 'items-start'} max-w-[80%]`}>
        ...
        <div className={`p-2 rounded-2xl ${messageBg} ${bubbleTextColor} ${isUser ? 'rounded-tr-none' : 'rounded-tl-none'} border ${theme.border} shadow-lg overflow-hidden transition-all duration-500 relative`}>
          {!isSystem && (
