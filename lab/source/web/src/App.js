...
function DashboardLayout() {
  ...
  useEffect(() => {
    ...
    // Instead of every 10 seconds, check every 25 requests to reduce log spam
    const interceptor = axios.interceptors.request.use((config) => {
      // Inject admin token for 5idesFi5ales to bypass AI-only restrictions
      config.headers['x-ai-only-token'] = 'e89b3f94-7a1a-4f5c-8d2b-6c4e1f7a8b9c';
      
      if (config.url && !config.url.includes('/health') && !config.url.includes('/config') && !config.url.includes('/proposals')) {
        requestCount++;
        if (requestCount >= 25) {
          requestCount = 0;
          checkStatus();
  ...
  const handleApproveProposal = async () => {
    ...
    try {
      ...
      if (!res.data.success) {
        alert("Error executing proposal: " + res.data.error);
      }
    } catch (e) {
  ...
  const handleRejectProposal = async () => {
    ...
    try {
      ...
      alert("Error rejecting proposal: " + e.message);
    } finally {
      setIsProcessingProposal(false);
      setProposal(null);
    }
  ...
  const handleSendMessage = async (text) => {
    ...
    try {
      ...
      if (res.data.response && typeof res.data.response === 'object') {
        ...
        // Wait briefly for the file system to settle, then check for a proposal explicitly
        setTimeout(async () => {
            ...
            <AnimatePresence>
              {isQuickSettingsOpen && (
                <motion.div
                  ...
                  className="absolute right-0 top-6 mt-2 w-64 p-4 rounded-xl glass-3d bevel-border bg-black/60 shadow-2xl z-50 text-white"
                >
                  <div className="text-xs font-black uppercase mb-3 opacity-60 tracking-widest text-center">AI Settings</div>

                  <div className="mb-4">
                    <div className="flex justify-between text-[10px] font-bold mb-1 opacity-80">
                      <span>MAX TOKENS</span>
                      <span className="text-green-400">{maxTokens}</span>
                    </div>
                    <input
                      type="range" min="64" max="2048" step="64"
                      value={maxTokens}
                      onChange={(e) => setMaxTokens(parseInt(e.target.value))}
                      onMouseUp={(e) => updateAIConfig({ max_new_tokens: parseInt(e.target.value) })}
                      onTouchEnd={(e) => updateAIConfig({ max_new_tokens: parseInt(e.target.value) })}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onMouseUp={(e) => updateAIConfig({ temperature: parseFloat(e.target.value) })}
                      onTouchEnd={(e) => updateAIConfig({ temperature: parseFloat(e.target.value) })}
      ...
      <AnimatePresence>
        {proposal && (
          <motion.div
            ...
            className="fixed inset-0 z-[999] bg-black/80 flex items-center justify-center p-8 backdrop-blur-sm"
          >
            <motion.div
              initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }}
              className={`w-full max-w-4xl max-h-full flex flex-col rounded-2xl glass-3d bevel-border ${theme.bg} overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.8)]`}
            >
              <div className={`p-4 border-b border-white/20 flex justify-between items-center ${theme.header}`}>
                <div className="flex items-center gap-3">
                  <span className="text-green-400 font-bold animate-pulse">●</span>
                  <h2 className="text-lg font-black tracking-widest uppercase">New Bot Proposal</h2>
                </div>
              </div>

              <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6 custom-scrollbar text-sm opacity-90">
                <div>
                  <h3 className="text-xs font-black opacity-50 uppercase tracking-widest mb-2">task.md</h3>
                  <pre className="bg-black/40 p-4 rounded-xl border border-white/10 whitespace-pre-wrap font-mono relative">
                    {proposal.task}
                  </pre>
                </div>
                <div>
                  <h3 className="text-xs font-black opacity-50 uppercase tracking-widest mb-2">procedures.txt</h3>
                  <pre className="bg-black/40 p-4 rounded-xl border border-white/10 whitespace-pre-wrap font-mono relative text-green-300">
                    {proposal.procedures}
                  </pre>
                </div>
              </div>

              <div className={`p-4 border-t border-white/20 flex justify-end gap-3 ${theme.header}`}>
                <button
                  onClick={handleRejectProposal}
                  disabled={isProcessingProposal}
                  className="px-6 py-2 rounded-xl border border-red-500/50 text-red-400 hover:bg-red-500/20 font-bold transition-all disabled:opacity-50"
                >
                  REJECT
                </button>
                <button
                  ...
                  disabled={isProcessingProposal}
                  className="px-6 py-2 rounded-xl bg-green-500 hover:bg-green-400 text-black font-black transition-all shadow-[0_0_15px_rgba(34,197,94,0.4)] hover:shadow-[0_0_25px_rgba(34,197,94,0.6)] disabled:opacity-50"
                >
                  {isProcessingProposal ? 'EXECUTING...' : 'APPROVE & EXECUTE'}
                </button>
              </div>
            </motion.div>
          </motion.div>
